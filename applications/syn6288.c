...
void SYN_FrameInfo(uint8_t Music, char *HZdata) // 对外接口
{
  ...
  rt_device_write(serial, 0, Frame_Info, (5 + HZ_Length + 1)); //  USART3_SendString(Frame_Info, 5 + HZ_Length + 1);
}
void YS_SYN_Set(char *Info_data,int_fast8_t length) // 对外接口
{
    rt_device_write(serial, 0, Info_data, length);   //USART3_SendString(Info_data, Com_Len);
...
{
    rt_err_t ret = RT_EOK;
    serial = rt_device_find(SYN6288_UART);/* 查找系统中的串口设备 Global handler*/
    if(!serial)
    {
...
//  config.bufsz     = 128;                   //64修改缓冲区 buff size 为 128
    rt_device_control(serial, RT_DEVICE_CTRL_CONFIG, &config);/*控制9600参数*/

    rt_sem_init(&rx_sem, "rx_sem", 0, RT_IPC_FLAG_FIFO);/* 初始化信号量 */
    rt_device_open(serial, RT_DEVICE_FLAG_INT_RX); /* 以中断接收及轮询发送模式打开串口设备 */
    rt_device_set_rx_indicate(serial, syn6288_input); /* 设置接收回调函数 */

    //    rt_device_write(serial, 0, str, (sizeof(str) - 1));  /* 发送字符串 */ -1 去掉\0
    rt_thread_t thread = rt_thread_create("SYN6288_serial", serial_thread_entry, RT_NULL, 1024, 25, 10); /* 创建 serial 线程 */
    /* 创建成功则启动线程 */
    if (thread != RT_NULL)
    {
